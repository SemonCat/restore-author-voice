...
`restore-author-voice` 是一個處理文章草稿與改寫的 agent skill。它的目標不是套上一種通用的「人味」，而是保留原作者真正有根據的聲音，並從最深層開始修復：篇章結構、段落推進、場域適配，最後才是表面語句。
...
- 清除沒人提出的反駁與只為否決而存在的假選項，同時保留真正的異議與決策選項
...
刪掉幾個常見套話，不會自動找回作者聲音。篇章結構如果仍是機器式模板，最後只會得到一篇比較短、但換誰署名都成立的文字。
...
- [`references/structure-and-discourse.md`](references/structure-and-discourse.md)：篇章結構、段落問題、節奏與修改深度
